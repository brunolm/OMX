...
if (!ManagerAddsSystem.OrkutManager.Startup) ManagerAddsSystem.OrkutManager.Startup = { };
if (!ManagerAddsSystem.OrkutManager.File) ManagerAddsSystem.OrkutManager.File = { };

ManagerAddsSystem.OrkutManager.Debug = {};
ManagerAddsSystem.OrkutManager.Debug.Log =
    function()
    {
        try
        {
...
ManagerAddsSystem.OrkutManager.Debug.ShowObj = function(o) {
    var t = [];
    for (var i in o)
    {
        try
...
ManagerAddsSystem.OrkutManager.File.ReadChrome = function(file)
{
    file = "chrome://orkutmanager/content/" + file;
...
ManagerAddsSystem.OrkutManager.LoadContent = function (document, file, name){
    var content = null;

    var source = ManagerAddsSystem.OrkutManager.File.ReadChrome(file);
    ...
    content.setAttribute("from", "gpm");
    if (typeof(name) != "undefined") content.setAttribute("name", "gpm-" + name);
    content.innerHTML = source;
    document.getElementsByTagName("head")[0].appendChild(content);
};

ManagerAddsSystem.OrkutManager.LoadTemplate = function (document, file, name)
{
    var source = ManagerAddsSystem.OrkutManager.File.ReadChrome(file);
...
ManagerAddsSystem.OrkutManager.LoadScript = function (url, context)
{
    var loader = Components.classes["@mozilla.org/moz/jssubscript-loader;1"].getService(Components.interfaces.mozIJSSubScriptLoader); 
    loader.loadSubScript(url, context);
}

ManagerAddsSystem.OrkutManager.GetDefaultPref = function(p,d)
{
    if (typeof(p) === "undefined") return d;
    return p;
};
ManagerAddsSystem.OrkutManager.LoadSettings = function(){
    ManagerAddsSystem.OrkutManager.UserSettings = ManagerAddsSystem.OrkutManager.PrefManager.Get("UserSettings") || "{}";
    
    var def = ManagerAddsSystem.OrkutManager.GetDefaultPref;
    var s = JSON.parse(ManagerAddsSystem.OrkutManager.UserSettings);
...
ManagerAddsSystem.OrkutManager.Initialize =
    function(e)
    {
        var contentWindow = e;
        var contentDocument = contentWindow.document;

        if (contentWindow != contentWindow.top) return;

        ManagerAddsSystem.OrkutManager.ContentWindow = contentWindow;

        if (/^https?:\/\/.*?\.orkut.*/i.test(contentWindow.location.href)
            || /manageraddons.com\/omx\/configext/i.test(contentWindow.location.href))
        {
            // Templates
            ManagerAddsSystem.OrkutManager.LoadTemplate(contentDocument, "templates/HelpBox.html", "helpbox-template");
            ManagerAddsSystem.OrkutManager.LoadTemplate(contentDocument, "templates/Config.html", "config-template");
        }

        // Content
        ManagerAddsSystem.OrkutManager.LoadContent(contentDocument, "css/default.css", "orkutmanager-css");
        ManagerAddsSystem.OrkutManager.LoadScript("chrome://orkutmanager/content/release/all.js");

        // Load user settings
        ManagerAddsSystem.OrkutManager.UserSettings = ManagerAddsSystem.OrkutManager.LoadSettings();
        //

        ManagerAddsSystem.OrkutManager.Startup(contentWindow, contentDocument, ManagerAddsSystem.OrkutManager.UnsafeWindow, ManagerAddsSystem.OrkutManager);
    };
...
gBrowser.addEventListener("DOMContentLoaded",
    function (e)
    {
        ...
        {
            ManagerAddsSystem.OrkutManager.Firefox = true;
            ManagerAddsSystem.OrkutManager.UnsafeWindow = e.target.defaultView.wrappedJSObject;
            ManagerAddsSystem.OrkutManager.Initialize(contentWindow);
        }
    },
    false);